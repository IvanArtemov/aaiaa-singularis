"""Entity-Centric Hybrid Extraction Pipeline with Semantic Retrieval and LLM Validation"""
...
class EntityCentricPipeline(BasePipeline):
    """
    Entity-Centric Hybrid Extraction Pipeline

    Universal system for extracting all entity types using semantic retrieval
    and LLM validation. Combines embeddings, vector search, and batch validation.

    Architecture:
    0. Parse PDF with GROBID → IMRAD sections
    0.5. Sentence Embeddings via Ollama (FREE - local) ✨
    1. LLM Keyword Generation via OpenAI (~$0.003/paper)
    4. Semantic Retrieval via ChromaDB (FREE - local)
    5. LLM Validation via OpenAI in batches (~$0.015/paper)
    6. Graph Assembly with heuristics (FREE)

    Cost: ~$0.018 per paper (embeddings FREE with Ollama!)
    Target Precision: ≥ 88%
    Target Recall: ≥ 82%
    Target F1-Score: ≥ 85%
    """
    ...
    def __init__(
        ...
        """
        Initialize Entity-Centric Pipeline

        Args:
            config: Configuration dictionary (optional if config_path provided)
            llm_provider: LLM provider for text generation ("openai" or "ollama")
            llm_model: LLM model name
            embedding_provider: Provider for embeddings (default: "ollama" for FREE local)
            config_path: Path to entity_centric_config.yaml (optional)
        """
        # Load config from file if provided
        if config_path:
            with open(config_path, 'r') as f:
                file_config = yaml.safe_load(f)
                config = file_config.get("entity_centric_pipeline", {})
        elif config is None:
            # Load default config
            default_config_path = Path(__file__).parent.parent / "config" / "entity_centric_config.yaml"
        ...
        # Initialize LLM adapter (for text generation: keywords, validation)
        self.llm_adapter = get_llm_adapter(llm_provider)

        # Initialize Embedding adapter (for embeddings: sentences, keywords)
        self.embedding_adapter = get_llm_adapter(embedding_provider)

        # Phase 0.5: Sentence Embedder (uses Ollama by default)
        embedding_config = config.get("embedding", {})
        self.sentence_embedder = SentenceEmbedder(
            llm_provider=embedding_provider,  # Use embedding provider (Ollama)
        ...
        self.keyword_generator = EntityKeywordGenerator(
            ...
            llm_model=keyword_config.get("model", llm_model),
            cache_size=keyword_config.get("cache_size", 128)
        )

        # Phase 4: Semantic Retriever
        retrieval_config = config.get("semantic_retrieval", {})
        self.retriever = SemanticRetriever(
            collection_name=retrieval_config.get("collection_name", "paper_segments"),
            persist_directory=retrieval_config.get("persist_directory", "./chroma_db"),
    ...
    def extract(
        ...
        """
        Extract entities and relationships using Entity-Centric approach
        ...
        # ========== PHASE 0.5: Sentence Embedding (~$0.0005) ==========

        if not parsed_doc.sentences:
            print("📝 Phase 0.5: Creating sentence embeddings...")
        ...
        # ========== PHASE 1: Keyword Generation (~$0.003) ==========

        print("🔑 Phase 1: Generating entity-specific keywords...")
        ...
        print("✅ Phase 5: Validating candidates with LLM...")
    ...
    def get_description(self) -> str:
        ...
        return (
            f"Entity-Centric Pipeline (Ollama Embeddings + Semantic Retrieval + LLM Validation). "
            f"Cost: ~$0.018/paper. Embeddings: {self.embedding_provider}, LLM: {self.llm_provider}."
        )

    def get_estimated_cost(self) -> float:
        """Get estimated cost per paper"""
        return 0.018  # FREE embeddings (Ollama) + OpenAI LLM calls
